...
# Добавляем путь к core (общая папка с BaseAgent)
core_path = Path(__file__).parent.parent.parent.parent / "src" / "core"
...
from agent import BaseAgent  # импортируем из core.agent (после добавления пути)
...
class WordStormCardAgent(BaseAgent):
    ...
    async def get_card(self, category: Optional[str] = None, language: str = "ru") -> Dict:
        ...
        # Если ничего нет, возвращаем заглушку
        new_card = {
